...
\begin{figure}
    ...
    \caption{$\alpha$ für verschiedene Temperaturen $T$} %Quelle einfügen
    \label{fig:alpha}
\end{figure}
...
%Tabelle einfügen mit \label{tab:C}
...
In Abbildung \ref{fig:C_V} ist zu erkennen, dass es bei der Messung einige Messfehler aufgetreten. Deswegen werden die Werte Nummer 4, 5 und 21 aus Tabelle \ref{tab:C} als Messfehler herausgenommen und erneut in einem Diagramm gegen die Temperatur $T$ aufgetragen.
...
%Tabelle mit \label{tab:Debye} hier einfügen
...
Die Debye Temperatur kann auch theoretisch bestimmt werden. Dazu wird Formel ... aus Abschnitt \ref{sec:debye} verwendet. Für die Geschwindigkeiten $v_l$ und $v_{tr}$ werden die Werte aus Quelle \cite{Anleitung} genutzt, diese lauten:
...
Die Teilchenanzahl $N_L$ kann wie folgt bestimmt werden:
...
$N_A = 6, \! 022 \cdot 10^ {23} \, \mathrm{mol}^{-1}$ ist dabei die Avogadro-Konstante. Für das Volumen $L^3$ gilt:

\begin{equation}
    L^3 = V_0 \cdot \frac{m}{M}
    \label{eq:L}
\end{equation}

Mit Hilfe von Formel ... und Einsetzung von \eqref{eq:N_L} und \eqref{eq:L} ergibt sich dann mit der Schallgeschwindigkeit $v_s$ für die Debye Frequenz $\omega_D$ folgende Formel:

\begin{equation}
    \omega_D = \sqrt[3]{\frac{6 \pi^2 \, v_s^3 \,  N_L}{L^3}} = \sqrt[3]{\frac{6 \pi^2 \, v_s^3 \, N_A}{V_0}}
    \label{eq:omega}
\end{equation}

Die Werte eingesetzt ergibt dann für die Debye-Frequenz folgenden Wert:

\begin{equation*}
    \omega_D = 4, \! 067 \cdot 10^{13} \, \mathrm{Hz} %Dieser Wert könnte falsch sein!
\end{equation*}
...
Dabei ist $\hbar = 1, \! 055 \cdot 10^{-34} \, \mathrm{Js}$ das reduzierte planksche Wirkungsquantum und $k_B = 1, \! 381 \cdot 10^{-23} \, \frac{\mathrm{J}}{\mathrm{K}}$ ist die Boltzmann-Konstante. Damit ergibt sich für die Debye-Temperatur $\theta_D$ ein theoretischer Wert von:

\begin{equation*}
    \theta_D = 310, \! 81 \, \mathrm{K} %Dieser Wert könnte falsch sein!
\end{equation*}